...
def get_stock_price():
    ...
    try:
        ...
        quote = get_quote_table(stock_symbol.upper())
...
def get_stock_history():
    stock_symbol = request.args.get('symbol')
    end_date = datetime.now().strftime('%Y-%m-%d')  # Gets today's date in the required format
    
    if not stock_symbol:
        return jsonify({'error': 'No stock symbol provided'}), 400

    try:
        # Here, we assume you want 5 years of data. Adjust as needed.
...
def get_stock_info():
    ...
    try:
        ...
        currency = quote_data.get('currency', 'USD')  # Assuming USD as default
